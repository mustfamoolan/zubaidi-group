        ...
        <div class="pt-5">
            <div class="panel">
                <div class="flex items-center justify-between mb-5">
                    <h5 class="font-semibold text-lg dark:text-white-light">تعديل الدور: {{ $role->display_name }}</h5>
                </div>
                ...
                <form action="{{ route('roles.update', $role->id) }}" method="POST" class="space-y-5">
                    ...
                    <div>
                        ...
                        <input id="name" name="name" type="text" value="{{ old('name', $role->name) }}" class="form-input" placeholder="أدخل اسم الدور بالإنجليزية" required />
                        <p class="text-xs text-gray-500 mt-1">مثال: admin, editor, user</p>
                    </div>

                    <div>
                        <label for="display_name">الاسم المعروض</label>
                        <input id="display_name" name="display_name" type="text" value="{{ old('display_name', $role->display_name) }}" class="form-input" placeholder="أدخل الاسم المعروض للدور" required />
                        <p class="text-xs text-gray-500 mt-1">مثال: مدير النظام، محرر، مستخدم</p>
                    </div>

                    <div>
                        <label for="description">الوصف</label>
                        <textarea id="description" name="description" class="form-textarea" placeholder="أدخل وصف الدور">{{ old('description', $role->description) }}</textarea>
                    </div>
                                    ...
                                    @foreach ($groupPermissions as $permission)
                                        <div class="mb-2">
                                            <label class="flex items-center cursor-pointer">
                                                <input type="checkbox" name="permissions[]" value="{{ $permission->id }}" class="form-checkbox" {{ in_array($permission->id, old('permissions', $rolePermissions)) ? 'checked' : '' }} />
