            ...
            <div class="px-5">
                <div class="md:absolute md:top-5 ltr:md:left-5 rtl:md:right-5">
                    <div class="flex items-center gap-2 mb-5">
                        ...
                        <a href="{{ route('companies.shipments.print-all', $company) }}" class="btn btn-success gap-2" target="_blank">
                        ...
                        <a href="{{ route('companies.shipments.create', $company) }}" class="btn btn-primary gap-2">
            ...
            Alpine.data('shipmentList', () => ({
                ...
                items: [
                    @foreach($shipments as $shipment)
                    {
                        ...
                        received_status: '{{ $shipment->received_status === "received" ? "مستلمة" : "غير مستلمة" }}',
                        entry_status: '{{ $shipment->entry_status === "entered" ? "داخلة" : "غير داخلة" }}',
                        entry_permit_status: '{{ $shipment->entry_permit_status === "received" ? "مستلم" : "غير مستلم" }}',
                        action: {{ $shipment->id }},
                    },
                    @endforeach
                ],
                    ...
                    this.datatable = new simpleDatatables.DataTable('#myTable', {
                        ...
                        columns: [{
                            ...
                            },
                            {
                                select: 1,
                                render: function(data, cell, row) {
                                    return '<a href="{{ route("companies.shipments.show", [$company, ""]) }}/' + row.cells[0].data + '" class="text-primary underline font-semibold hover:no-underline">' + data + '</a>';
                                }
                            },
                            {
                                select: 3,
                            ...
                            },
                            {
                                ...
                                render: function(data, cell, row) {
                                    ...
                                    let editUrl = '/companies/{{ $company->id }}/shipments/' + shipmentId + '/edit';
                                    let showUrl = '/companies/{{ $company->id }}/shipments/' + shipmentId;
                                    return `<div class="flex gap-4 items-center">
                                                ...
                                                <button type="button" class="hover:text-danger" @click="deleteRow(${shipmentId})">
                                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5">
                                                        <path d="M20.5001 6H3.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"></path>
                                                        <path d="M18.8334 8.5L18.3735 15.3991C18.1965 18.054 18.108 19.3815 17.243 20.1907C16.378 21 15.0476 21 12.3868 21H11.6134C8.9526 21 7.6222 21 6.75719 20.1907C5.89218 19.3815 5.80368 18.054 5.62669 15.3991L5.16675 8.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"></path>
                                                        <path opacity="0.5" d="M9.5 11L10 16" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"></path>
                ...
                deleteRow(item) {
                    if (confirm('هل أنت متأكد من حذف الشحنة المحددة؟')) {
                        if (item) {
                            let form = document.createElement('form');
                            form.method = 'POST';
                            form.action = `{{ route('companies.shipments.destroy', [$company, '']) }}/${item}`;
                            ...
                            if (this.selectedRows.length === 0) {
                                alert('يرجى تحديد شحنة واحدة على الأقل للحذف');
                                return;
                            }
